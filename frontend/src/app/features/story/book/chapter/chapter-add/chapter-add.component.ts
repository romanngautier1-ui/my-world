...
import { ChapterCreateDto } from '../../../../../shared/dtos/chapter.dto';
...
@Component({
  ...
  styleUrl: './chapter-add.component.css',
})
export class ChapterAddComponent {
  ...
  onSubmit() { 
    ...
    if (isFromValid) {
      const newChapter: Omit<ChapterCreateDto, 'id'> = {
      ...
      this.#chapterService.addChapter(newChapter).subscribe((createdChapter) => {
        this.router.navigate(['/books', this.bookId, 'chapters', createdChapter.id]);
      });
    }
  }
}
